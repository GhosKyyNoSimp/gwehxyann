...
if (chats.startsWith("woy")){
  ...
  conn.sendMessage(from, { caption: teks, video : {url:'https://mmg.whatsapp.net/d/f/Ano5cGYOFQnC51uJaqGBWiCrSJH1aDCi8-YPQMMb1N1y.enc'}, templateButtons: buttonsDefault,mimetype:'application/mp4',footer: 'Ghos - Bid',mention: [sender]} )
				break
				case prefix+'catbox':
				if (args.length < 2) return reply(`Kirim perintah ${command} Link\nContoh : ${command} https://file.catbox.moe/gjiuhnk.mp4`)
				conn.sendMessage(from, { caption: 'nih kak', video: { url : `${q}` }, templateButtons: bunbun }, {quoted: msg})
...
case prefix+'bc': case prefix+'broadcast': case 'broadcast':
			    if (!isOwner) return reply(mess.OnlyOwner)
		            if (args.length < 2) return reply(`Masukkan isi pesannya`)
                            ...
                            for (let i of data)   {          
							conn.sendMessage(i.id, { text: `${q}`, templateButtons: bece, footer:'Ghos Bid Broadcast'})
                               await sleep(1000)
                            }
                            break
			case prefix+'addprem':
